...
function promptOrder() {
  rl.question('\nEnter order (buy/sell amount price): ', async (input) => {
    ...
    client.map(SERVICE_NAME, order, { timeout: 5000 }, (err, data) => {
      if (err) {
        console.error(`[${reqId}] ERROR calling ${SERVICE_NAME}:`, err.message);
      } else {
        Object.entries(data).forEach(([peer, res]) => {
          if (res && res.error) {
            console.error(`[${reqId}] ERROR from ${peer}:`, res.error);
          } else {
            logWithRequestId(reqId, `RESPONSE FROM ${peer}: ${JSON.stringify(res)}`);
          }
        });
      }
